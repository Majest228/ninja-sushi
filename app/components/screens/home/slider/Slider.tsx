          ...
          >
            {slides.map((slide) => (
              <div
                ...
                key={slide.id}
                style={{ backgroundImage: `url(${slide.banner.src})`, backgroundSize: "cover" }}
              >
                <div className={styles.slider__slide__content}>
                  <h3>{slide.title}</h3>
                  <p>{slide.desciption}</p>
                  <Link className={styles.slider__slide__content__link} href={"/news"}>
                    Перейти к новости
                  </Link>
                </div>
              </div>
            ...
            {slides.map((slide) => (
              <button
                ...
                className={classNames(
                  ...
                  slide.id - 1 == currentIndex ? [styles.slider__dots__dot__active] : ""
                )}
